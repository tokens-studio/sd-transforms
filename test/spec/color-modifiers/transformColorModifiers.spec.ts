...
describe('transform color modifiers', () => {
  describe('lighten', () => {
    it('supports lighten color modifiers in all 4 spaces', () => {
      ...
      expect(transformColorModifiers(token(ColorSpaceTypes.HSL), mathFractionDigits)).to.equal(
        'hsl(0 50.6% 60.6%)',
      );
      expect(transformColorModifiers(token(ColorSpaceTypes.LCH), mathFractionDigits)).to.equal(
        'lch(57.7 47.5 29.7)',
      );
      expect(transformColorModifiers(token(ColorSpaceTypes.P3), mathFractionDigits)).to.equal(
        'color(display-p3 0.76 0.435 0.422)',
      );
      expect(transformColorModifiers(token(ColorSpaceTypes.SRGB), mathFractionDigits)).to.equal(
        'rgb(80.5% 40.7% 40.7%)',
      );
      // without space, return original
      expect(transformColorModifiers(token(''), mathFractionDigits)).to.equal('#C14242');
    });
  ...
  describe('mix', () => {
    it('supports mix color modifiers in all 4 spaces', () => {
      ...
      // without space, return original
      expect(transformColorModifiers(token(''), mathFractionDigits)).to.equal('#000000');
    });
    ...
    it('supports mix modifier value calculations', () => {
      ...
      expect(transformColorModifiers(token(''), mathFractionDigits)).to.equal('#FFFFFF');
      expect(
        transformColorModifiers(token(ColorSpaceTypes.SRGB, 'hex'), mathFractionDigits),
      ).to.equal('#759ae6');
    });
  ...
  describe('overriding of math fraction digits options', () => {
    it('transform options color modify math fraction digits override default math fraction digits', () => {
      ...
      expect(
        transformColorModifiers(token(ColorSpaceTypes.SRGB), mathFractionDigits, {
          format: 'srgb',
          mathFractionDigits: 10,
        }),
    ...
    it('token modify math fraction digits override default math fraction digits', () => {
      ...
      expect(
        transformColorModifiers(tokenLessPrecise(ColorSpaceTypes.SRGB), mathFractionDigits),
      ).to.equal('rgb(44.289% 59.483% 89.871%)');
      expect(
        transformColorModifiers(tokenMorePrecise(ColorSpaceTypes.SRGB), mathFractionDigits),
      ).to.equal('rgb(44.287% 59.482% 89.87%)');
    });
  ...
  it('allows passing an output format', () => {
    ...
    expect(transformColorModifiers(token(), mathFractionDigits)).to.equal('rgb(60.5% 20.7% 20.7%)');
    // output to hsl
    expect(transformColorModifiers(token(ColorSpaceTypes.HSL), mathFractionDigits)).to.equal(
      'hsl(0 49% 40.6%)',
    );
    // output to hex
    expect(transformColorModifiers(token('hex'), mathFractionDigits)).to.equal('#9a3535');
  });
  ...
  it('allows passing an output format as an option', () => {
    ...
    expect(
      transformColorModifiers(token(ColorSpaceTypes.HSL), mathFractionDigits, { format: 'hex' }),
    ).to.equal('#9c3333');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.LCH), mathFractionDigits, { format: 'hex' }),
    ).to.equal('#983735');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.P3), mathFractionDigits, { format: 'hex' }),
    ).to.equal('#9b3535');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.SRGB), mathFractionDigits, { format: 'hex' }),
    ).to.equal('#9a3535');

    // uses hsl override for output format
    expect(
      transformColorModifiers(token(ColorSpaceTypes.HSL), mathFractionDigits, { format: 'hsl' }),
    ).to.equal('hsl(0 50.6% 40.6%)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.LCH), mathFractionDigits, { format: 'hsl' }),
    ).to.equal('hsl(0.85 47.9% 40.3%)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.P3), mathFractionDigits, { format: 'hsl' }),
    ).to.equal('hsl(0.052 49% 40.7%)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.SRGB), mathFractionDigits, { format: 'hsl' }),
    ).to.equal('hsl(0 49% 40.6%)');

    // uses lch override for output format
    expect(
      transformColorModifiers(token(ColorSpaceTypes.HSL), mathFractionDigits, { format: 'lch' }),
    ).to.equal('lch(37.8 51 29.8)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.LCH), mathFractionDigits, { format: 'lch' }),
    ).to.equal('lch(37.7 47.5 29.7)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.P3), mathFractionDigits, { format: 'lch' }),
    ).to.equal('lch(37.9 49.4 29.4)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.SRGB), mathFractionDigits, { format: 'lch' }),
    ).to.equal('lch(37.8 49.4 29.4)');

    // uses p3 override for output format
    expect(
      transformColorModifiers(token(ColorSpaceTypes.HSL), mathFractionDigits, { format: 'p3' }),
    ).to.equal('color(display-p3 0.565 0.23 0.216)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.LCH), mathFractionDigits, { format: 'p3' }),
    ).to.equal('color(display-p3 0.551 0.241 0.224)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.P3), mathFractionDigits, { format: 'p3' }),
    ).to.equal('color(display-p3 0.56 0.235 0.222)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.SRGB), mathFractionDigits, { format: 'p3' }),
    ).to.equal('color(display-p3 0.56 0.235 0.222)');

    // uses srgb override for output format
    expect(
      transformColorModifiers(token(ColorSpaceTypes.HSL), mathFractionDigits, { format: 'srgb' }),
    ).to.equal('rgb(61.2% 20.1% 20.1%)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.LCH), mathFractionDigits, { format: 'srgb' }),
    ).to.equal('rgb(59.5% 21.5% 21%)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.P3), mathFractionDigits, { format: 'srgb' }),
    ).to.equal('rgb(60.6% 20.8% 20.7%)');
    expect(
      transformColorModifiers(token(ColorSpaceTypes.SRGB), mathFractionDigits, { format: 'srgb' }),
    ).to.equal('rgb(60.5% 20.7% 20.7%)');
  ...
  it('can convert from a non-srgb space to srgb space to then format it as a hex color (which is fundamentally rgb)', () => {
    ...
    expect(transformColorModifiers(token, mathFractionDigits)).to.equal('#983735');
  });
  ...
  it('supports UIColor(r,g,b,a) format', () => {
    ...
    expect(transformColorModifiers(token, mathFractionDigits)).to.equal('rgb(50% 0% 25% / 0.5)');
  });
  ...
  it('should return undefined if the transformation needs to be deferred', () => {
    ...
    expect(transformColorModifiers(token, mathFractionDigits)).to.be.undefined;
  });
});
